    ...
    for acc in accounts:
        ...
        for tran in transaction:
            if tran.account_id == acc.id:
                trans["account number" + " : " + acc.account_number].append(
                    {
                        "action": tran.action,
                        "amount": tran.amount,
                        "balance before": tran.balance_before,
                        "balance after": tran.balance_after,
                        "time stamp": tran.timestamp,
                    }
                )
...
def ExportTransactions(accno_last4, CustomerID, role):
    ...
    account = Account.query.filter(
        ...
        Account.account_number.like(f"%{accno_last4}"),
    ).first()
